...
const createDefaultLevels = (height: number): ShelfLevel[] => [
    { id: crypto.randomUUID(), elevation: 200, material: 'Wood' },
    { id: crypto.randomUUID(), elevation: Math.min(height - 200, 1000), material: 'Wood' },
    { id: crypto.randomUUID(), elevation: Math.min(height - 200, 1800), material: 'Wood' },
];
    ...
    const addLevel = useCallback((moduleId: string) => {
        setConfig(prev => {
            ...
            if (sortedLevels.length > 0) {
                ...
                insertElevation = 200; maxGapSize = Infinity;
            }
            ...
            if (insertElevation < 0) insertElevation = 100;
            if (insertElevation > prev.height) insertElevation = prev.height - 100;
